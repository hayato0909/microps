...
arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER || msg->hdr.hln != ETHER_ADDR_LEN) {
        errorf("hrd: 0x%04x", ntohs(msg->hdr.hrd));
        return;
    }
    if (ntoh16(msg->hdr.pro) != ARP_PRO_IP || msg->hdr.pln != IP_ADDR_LEN) {
        errorf("pro: 0x%04x", ntohs(msg->hdr.pro));
        return;
    }
...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (iface->family != NET_IFACE_FAMILY_IP) {
        debugf("unsupported address family");
        return ARP_RESOLVE_ERROR;
    }
    mutex_lock(&mutex);
    cache = arp_cache_select(pa);
    ...
    mutex_unlock(&mutex);
    debugf("pa=%s, ha=%s", ip_addr_ntop(&pa, addr1, sizeof(addr1)), ether_addr_ntop(ha, addr2, sizeof(addr2)));
...
arp_init(void)
{
    ...
    if (net_protocol_register(NET_PROTOCOL_TYPE_ARP, arp_input) == -1) {
        errorf("net_protocol_register() failture");
        return -1;
    }
